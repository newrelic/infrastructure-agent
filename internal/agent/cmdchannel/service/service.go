...
func (s *srv) handle(ctx context.Context, c commandapi.Command, initialFetch bool, agentID entity.ID) {
	if s.wasACKed(c, agentID) {
		ccsLogger.
			...
			Tracef("skipping cmd already ACKed: %s, hash: %s, args: %s", c.Name, c.Hash, string(c.Args))
		return
	}
	...
	if s.requiresAck(c, agentID) {
		ccsLogger.
			WithField(config.TracesFieldName, config.FeatureTrace).
			Tracef("triggering ACK for cmd: %s, hash: %s, args: %s", c.Name, c.Hash, string(c.Args))
