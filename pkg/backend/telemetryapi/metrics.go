...
func writeTimestampInterval(w *JSONFieldsWriter, timestamp time.Time, interval time.Duration) {
	if !timestamp.IsZero() {
...
func (m Count) writeJSON(buf *bytes.Buffer) {
	...
	if nil != m.Attributes {
		w.WriterField("attributes", Attributes(m.Attributes))
