...
var diskRegex = regexp.MustCompile("^/dev/(disk[0-9]+)")
...
func (self *DiskMonitor) Sample() (result *DiskSample, err error) {
	...
	for _, ss := range samples {
		...
		disk, err := diskFromDevice(ss)
		if err != nil {
			return nil, err
		}


		// APFS filesystems partitions share total size and free space
...
func diskFromDevice(ss *storage.Sample) (string, error) {
	matches := diskRegex.FindStringSubmatch(ss.Device)
	if len(matches) != 2 {

		return "", fmt.Errorf("cannot match disk from device %s", ss.Device)
	}
