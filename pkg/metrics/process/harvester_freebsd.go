...
func newHarvester(ctx agent.AgentContext) *freebsdHarvester {
	cfg := ctx.Config()
	...
	processRetriever := s.cache.ProcessById

	return &freebsdHarvester{
...
// freebsdHarvester is a Harvester implementation that uses various darwin sources and manages process caches
type freebsdHarvester struct {
	privileged           bool
...
var _ Harvester = (*freebsdHarvester)(nil) // static interface assertion

// Pids returns a slice of process IDs that are running now
func (*freebsdHarvester) Pids() ([]int32, error) {
	return process.Pids()
}
...
func (fh *freebsdHarvester) Do(pid int32, elapsedSeconds float64) (*types.ProcessSample, error) {
	proc, err := fh.processRetriever(pid)
	if err != nil {
		return nil, errors.Wrap(err, "can't create process")
	}

	procSnapshot, err := snapshot.getFreebsdProcess(proc, fh.privileged)
	if err != nil {
		return nil, errors.Wrap(err, "can't create process")
	}

	// We don't need to report processes which are not using memory. This filters out certain kernel processes.
	if !fh.disableZeroRSSFilter && procSnapshot.VmRSS() == 0 {
	...
	if err = fh.populateStaticData(sample, procSnapshot); err != nil {
		return nil, errors.Wrap(err, "can't populate static attributes")
	}

	if err = fh.populateGauges(sample, procSnapshot); err != nil {
	...
	sample.ProcessDisplayName = fh.determineProcessDisplayName(sample)
...
func (fh *freebsdHarvester) populateStaticData(sample *types.ProcessSample, processSnapshot Snapshot) error {
	var err error
...
func (fh *freebsdHarvester) populateGauges(sample *types.ProcessSample, process Snapshot) error {
	var err error
...
func (fh *freebsdHarvester) determineProcessDisplayName(sample *types.ProcessSample) string {
	displayName := sample.CommandName
	if serviceName, ok := fh.serviceForPid(int(sample.ProcessID)); ok && len(serviceName) > 0 {
