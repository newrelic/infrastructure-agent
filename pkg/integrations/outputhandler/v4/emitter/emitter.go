...
func (e *VersionAwareEmitter) Emit(definition integration.Definition, extraLabels data.Map, entityRewrite []data.EntityRewrite, integrationJSON []byte) error {
	...
	protocolVersion, err := protocol2.VersionFromPayload(integrationJSON, e.forceProtocolV2ToV3)
	if err != nil {
	...
	pluginDataV3, err := protocol2.ParsePayload(integrationJSON, protocolVersion)
	if err != nil {
