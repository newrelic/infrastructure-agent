...
import (
	ctx2 "context"
	"fmt"
	logs2 "github.com/newrelic/infrastructure-agent/pkg/integrations/execution/v4/logs"
	"io/ioutil"
	"os"
	"time"
...
func NewFBSupervisor(fbIntCfg FBSupervisorConfig, cfgLoader *logs2.CfgLoader, agentIDNotifier id.UpdateNotifyFn, notifier hostname.ChangeNotifier, sendEventFn SendEventFn) *Supervisor {
	return &Supervisor{
		...
		parseOutputFn:          logs2.ParseFBOutput,
	}
}
...
func buildFbExecutor(fbIntCfg FBSupervisorConfig, cfgLoader *logs2.CfgLoader) func() (Executor, error) {
	return func() (Executor, error) {
		...
		if (externalCfg != logs2.FBCfgExternal{} && externalCfg.ParsersFilePath != "") {
...
func listenRestartRequests(cfgLoader *logs2.CfgLoader) func(ctx ctx2.Context, signalRestart chan<- struct{}) {
	cw := logs2.NewConfigChangesWatcher(cfgLoader.GetConfigDir())
