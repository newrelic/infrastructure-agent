...
echo "%__gpg_sign_cmd   %{__gpg} gpg --verbose --no-armor --batch --passphrase ${GPG_PASSPHRASE} --no-secmem-warning -u "%{_gpg_name}" -sbo %{__signature_filename} %{__plaintext_filename}" >> ~/.rpmmacros
