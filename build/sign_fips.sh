...
for rpm_file in $(find -regex ".*\.\(rpm\)");do
  ...
  ../build/sign_rpm_fips.exp $rpm_file ${GPG_PASSPHRASE}
...
chmod +x ../build/sign_deb_fips.exp
...
../build/sign_deb_fips.exp $deb_file ${GPG_PASSPHRASE} ${GPG_MAIL}
...
chmod +x ../build/sign_tar_fips.exp
